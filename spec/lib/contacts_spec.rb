...
describe "when merging contacts" do
  let(:master) { FactoryGirl.create(:contact, title: "Master Contact", account: FactoryGirl.create(:account),
      source: "Master Source", background_info: "Master Background Info", tag_list: 'tag1, tag2, tag3') }
  let(:duplicate) { FactoryGirl.create(:contact, title: "Duplicate Contact", account: FactoryGirl.create(:account),
  ...
  describe "a merge failure" do
    ...
    it "should rollback the transaction", testing_transactions: true do
      ...
      expect(duplicate).to receive(:destroy).and_raise(StandardError, "merge error")
